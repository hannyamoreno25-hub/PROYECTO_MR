...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    // 1.2 Encender las cámaras nuevas (si existen en la pantalla actual)
    try {
        ...
        if (videoJornada || videoVisita) {
            ...
            if (videoJornada) {
                ...
                videoJornada.play(); // Forzamos reproducción
            }
            if (videoVisita) {
                videoVisita.srcObject = stream;
                videoVisita.play(); // Forzamos reproducción
            }
        }
    } catch (err) {
    ...
    // 1.3 Cargar datos para otras pantallas (Configuración / Visitas)
    if (document.getElementById('tablaVisitasBody')) {
    ...
    if (datos && document.getElementById('confNombre')) {
        document.getElementById('confNombre').value = datos.nombre || '';
        document.getElementById('confFiscal').value = datos.fiscal || '';
        document.getElementById('confID').value = datos.idTrabajador || '';
        document.getElementById('confPuesto').value = datos.puesto || '';
    }
    if (foto && document.getElementById('profilePreview')) {
...
function tomarFoto(idVideo, idCanvas, idFoto) {
    ...
    foto.style.display = 'block'; // Muestra foto
...
// ==========================================
// 5. ENVÍO A GOOGLE SHEETS (VERSIÓN CON DETECTOR DE ERRORES)
// ==========================================
async function enviarAGoogle(datos) {
    const payload = {
        action: "registrar_actividad",
        usuario: localStorage.getItem('usuario_nombre') || 'Desconocido',
    ...
    try {
        ...
        if(!result.ok) {
            // AQUÍ ESTÁ LA MAGIA: Ahora nos dirá el error exacto
...
// ==========================================
// 7. FUNCIONES DE TUS OTRAS PANTALLAS (Agendar / Configuración)
// ==========================================
let visitasAgendadas = JSON.parse(localStorage.getItem('registroVisitas')) || [];

function agendarVisita() {
    const sucursal = document.getElementById('sucursalSelectAgenda').value;
    ...
    visitasAgendadas.push(nuevaVisita);
    localStorage.setItem('registroVisitas', JSON.stringify(visitasAgendadas));
    actualizarTablaVisitas();
    ...
    visitasAgendadas.forEach((visita) => {
        const fila = `
            <tr>
                ...
                <td>${visita.fecha}</td>
                <td><strong>${visita.estado}</strong></td>
            </tr>
        `;
...
function exportarRegistro() {
    alert("Función de exportación lista para vincular a base de datos.");
}
...
function guardarConfiguracion() {
    const usuario = {
        ...
        fiscal: document.getElementById('confFiscal').value,
        idTrabajador: document.getElementById('confID').value,
        puesto: document.getElementById('confPuesto').value
    };
